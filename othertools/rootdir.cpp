...
static void patch_init_rc(const char *src, const char *dest, const char *tmp_dir) {
    ...
    for (auto &script : rc_list) {
        ...
        replace_all(script, "${TEMP}", tmp_dir);
        fprintf(rc, "\n%s\n", script.data());
    }
    rc_list.clear();
    ...
    LOGD("Inject rcds: [%s] [%s]\n", pfd_svc, ls_svc);
    fprintf(rc, RCDS, tmp_dir, pfd_svc, ls_svc);
...
void SARBase::backup_files() {
    ...
    if (access("/.backup/.rc", R_OK) == 0)
       rc_cfg = mmap_data("/.backup/.rc");
    else if (access("/data/.backup/.magisk", R_OK) == 0)
        rcds_cfg = mmap_data("/data/.backup/.rcds");
}
...
static void extract_files(bool sbin) {
    const char *m32 = sbin ? "/sbin/rc32.xz" : "rc32.xz";
    const char *m64 = sbin ? "/sbin/rc64.xz" : "rc64.xz";

    if (access(m32, F_OK) == 0) {
        auto rc = mmap_data(m32);
        unlink(m32);
        int fd = xopen("rc32", O_WRONLY | O_CREAT, 0755);
        unxz(fd, rc.buf, rc.sz);
        close(fd);
        patch_socket_name("rc32");
    }
    if (access(m64, F_OK) == 0) {
        auto rc = mmap_data(m64);
        unlink(m64);
        int fd = xopen("rc64", O_WRONLY | O_CREAT, 0755);
        unxz(fd, rc.buf, rc.sz);
        close(fd);
        patch_socket_name("rc64");
        xsymlink("./rc64", "rc");
    } else {
        xsymlink("./rc32", "rc");
    }
...
void RootFSInit::prepare() {
    self = mmap_data("/init");
    rcds_cfg = mmap_data("/.backup/.rcds");

    LOGD("Restoring /init\n");
    rename(backup_init(), "/init");
}

#define PRE_TMPDIR "/rc-temp"
...
void MagiskInit::patch_rw_root() {
    ...
    // Dump rc as rcds
    int fd = xopen("/sbin/rctds", O_WRONLY | O_CREAT, 0755);
    write(fd, self.buf, self.sz);
    close(fd);
}

int rcds_proxy_main(int argc, char *argv[]) {
    setup_klog();
    LOGD("%s\n", __FUNCTION__);
    ...
    // Tell rcd to remount rootfs
    setenv("REMOUNT_ROOT", "1", 1);
    execv("/sbin/xcore", argv);
    return 1;
}
